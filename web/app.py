...
from benefactors.models import User, Post, PostComment, ChatChannel, ChatMessages,  Notification, statusEnum
...
def seed_db():
    ...
    db.session.add(User(username="sina123", first_name="Sina", last_name="Lin", email="1@1.com", phone_number="1111", postal_code="V3E3B5", password="$2b$12$ppAg.mOnlIo15d0m7gPYr.1LZaUvuO29JVVBkv6bkQzQz6zK.f66y"))
    ...
    db.session.add(Post(title="Please Help Get My Groceires", category="GROCERY", description="Would really appreciate someone picking up some essential groceries. Eggs, bread, milk, that kind of thing.", user_id=2, volunteer=1, status = statusEnum.TAKEN))
    db.session.add(Post(title="Need to Get My Medications", category="GROCERY", description="I'm a little scared to step out to get meds from my local pharmacy, would really appreciate if someone could pick them up for me", user_id=2, volunteer=1, status = statusEnum.TAKEN))
