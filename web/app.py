...
def seed_db():
    ...
    db.session.add(Post(title="CMPT-454 textbook", description="A CMPT book", author=users[1]))
    db.session.add(Post(title="Physics", description="A Book", author=users[2]))
    db.session.add(Post(title="SpaceX", description="A space exploration company", author=users[2]))
